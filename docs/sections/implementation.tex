...
Model je kompilovaný s účelovou funkciou \emph{kategorická krížová entropia} (loss=categorical_crossentropy).
...
Ukladanie prebieha jednoducho zavolaním metódy \emph{save} a \emph{save_weights} nad natrénovaným modelom a definovaním
cesty pre uložené súbory.
Následne je potrebné uložiť ešte aj celú reláciu trénovania (pomocou \emph{freeze_session}) a nakoniec zapísať celú
štruktúru ako graf s použitím \emph{write_graph}.
